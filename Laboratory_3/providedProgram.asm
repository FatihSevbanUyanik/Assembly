...
la $a0, duplicate_iteratively
li $v0,4	
syscall	

la $a0, duplicate_recursively
li $v0,4	
syscall	
...
Insert_n:
	...
	addi $s0, $a0, 0 #get pointer to list
	addi $s1, $a1, 0 #get data
	addi $s2, $a2, 0 #get n

	bne $t9, $zero, is_add_to_head
	
	# List is Empty
	
	# allocating 8 byte from heap.
	li $v0,9
	addi $a0,$0, 8
	syscall
	
	# assigning adress and value 
	# of allocated space to $s0. 
	addi $s0, $v0, 0 
	sw $s1, 4($s0)
	sw $zero, 0($s0)
...
is_add_to_head:
	bne $s2, 1, insert_to_tail
	
	# inserting elemnt to head.
	# allocating 8 byte from heap.
	li $v0,9
	addi $a0,$0, 8
	syscall
...
insert_to_tail: 
	slt $s3, $t9, $s2
	bne $s3, 1, insert_to_n 
	
	# inserting node to tail
	addi $s3, $zero, 1
	move $s4, $s0
	
	loop_1: beq $s3, $t9, exit_1
		lw $s4, 0($s4)
		addi $s3, $s3, 1	
		j loop_1		
	exit_1: 
...
insert_to_n: 
	addi $s4, $zero, 1
	slt $s3, $s2, $s4
	beq $s3, 1, output_invalid_index

	# inserting a node between head and tail
	addi $s3, $zero, 1
	move $s4, $s0
	addi $s5, $s2, -1
	
	loop_2: beq $s3, $s5, exit_2
		lw $s4, 0($s4)
		addi $s3, $s3, 1	
		j loop_2		
	exit_2: 
...
output_invalid_index:
	# outputting negative index.
	addi $v0, $zero, 4
 	la $a0, insert_invalid_index
 	syscall 
...
insert_invalid_index: .asciiz "The position index is invalid. \n"

duplicate_iteratively: .asciiz "7 - Duplicate Array iteratively. \n"

duplicate_recursively: .asciiz "8 - Duplicate Array recursively. \n"

delete_count: .asciiz "Deleted Item Count: "
